...
- 四个轮子的转动由 `myrobot_simulation/scripts/mecanum_sim_driver.py` 根据 `/cmd_vel` 计算并发布；
...
```bash
roslaunch myrobot_simulation full_simulation.launch launch_rviz:=false
```
...
```bash
roslaunch myrobot_simulation full_simulation.launch start_test_motion:=true
```
...
`myrobot_simulation/scripts/mecanum_sim_driver.py` 使用麦克纳姆轮逆运动学，把同一个 `/cmd_vel` 转成四个轮子的角速度：
...
```text
myrobot_simulation/config/simulation_params.yaml
```
